// @ts-nocheck
export function scoreItem(query, label) {
  const q = query.toLowerCase().trim();
  if (!q) return { score: 0, firstHitIndex: -1 };
  const tokens = q.split(/\s+/).filter(Boolean);
  if (tokens.length === 0) return { score: 0, firstHitIndex: -1 };
  const lo = label.toLowerCase();
  const firstHitIndex = lo.indexOf(tokens[0]);
  if (lo.startsWith(q)) return { score: 3, firstHitIndex: lo.indexOf(q) };
  if (lo.includes(q)) return { score: 2, firstHitIndex: lo.indexOf(q) };
  const allPresent = tokens.every((t) => lo.includes(t));
  if (allPresent) return { score: 1, firstHitIndex };
...
function score(query, entry) {
  ...
  let s = scoreItem(query, entry.title).score * 10;
  for (const t of tokens) {
    if (entry.tags.some((tag) => tag.toLowerCase().includes(t))) s += 5;
    if (body.includes(t)) s += 1;
    else if (title.includes(t)) {} // already scored via scoreItem
