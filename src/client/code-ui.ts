// @ts-nocheck
// File-upload handler, exec block enlarge modal, image lightbox.
import { loadPyodideRuntime } from "./execution";
...
document.addEventListener("change", async (e) => {
  ...
  try {
    ...
    fileListEl.innerHTML = loaded.map((name) => `<span class="upload-file-tag">${escapeHtml(name)}</span>`).join("");
  } catch (err) {
