// @ts-nocheck
// Page nav, font-size cycling, focus mode, TOC scroll spy, resize
// handles, sidebar collapse persistence, resource browser tabs,
// enter-folder feature.
import { settings, saveSettings, applySettings, FONT_SIZES } from "./settings";
...
export function navigateToPage(offset) {
  const links = getNavLinks();
...
export function cycleFontSize(dir) {
...
async function loadResourceTab(tab) {
  ...
  try {
    ...
    html += '</ul></nav>';
    const currentNav = sidebar?.querySelector(".sidebar-nav");
    if (currentNav) currentNav.outerHTML = html;
  } catch {}
}
...
function applyEnteredFolder() {
  ...
  for (const ancestorLi of ancestorLis) {
    ...
    for (const sibling of parentUl.children) {
      if (sibling !== ancestorLi) sibling.classList.add("entered-folder-hidden");
    }
