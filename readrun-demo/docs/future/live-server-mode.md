...
**Status: Implemented** -- select "Live Server" from the TUI.

Live server mode runs Python code blocks natively on your machine via `uv run`, instead of in the browser via Pyodide. It also enables a files panel (`.readrun/files/`), pre-seeded data files, and inline rendering of generated images.
